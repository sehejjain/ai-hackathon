...
spendconscience-agents/
├── src/
│   ├── /spendconscience              # Agent configurations
├── apps/
...
- `src/spendconscience/.env`: Inkeep CLI environment variables
...
Your graphs are defined in `src/spendconscience/weather.graph.ts`. The default setup includes:

- **Weather Graph**: A graph that can forecast the weather in a given location.

Your inkeep configuration is defined in `src/spendconscience/inkeep.config.ts`. The inkeep configuration is used to configure defaults for the inkeep CLI. The configuration includes:
...
1. Edit `src/spendconscience/weather.graph.ts`
...
- Ensure you are runnning commands from `cd src/spendconscience`.
- Validate the `inkeep.config.ts` file has the correct api urls.
- Validate that the `.env` file in `src/spendconscience` has the correct `DB_FILE_NAME`.
